...
#include "blf.h"
#include "include/CBcryptPBKDF.h"

/*
 * Keygate portability shims. Upstream uses OpenBSD's <sha2.h> and <util.h>;
 * on macOS we map SHA-512 onto CommonCrypto and provide our own zeroing.
 */
#define SHA2_CTX                CC_SHA512_CTX
#define SHA512_DIGEST_LENGTH    CC_SHA512_DIGEST_LENGTH
#define SHA512Init              CC_SHA512_Init
#define SHA512Final(d, c)       CC_SHA512_Final((d), (c))
#define SHA512Update(c, p, n)   CC_SHA512_Update((c), (p), (CC_LONG)(n))

static void keygate_bzero(void *buf, size_t len)
{
	volatile unsigned char *p = (volatile unsigned char *)buf;
	while (len--)
		*p++ = 0;
}
#define explicit_bzero keygate_bzero
...
int
keygate_bcrypt_pbkdf(const char *pass, size_t passlen, const uint8_t *salt, size_t saltlen,
